...
<div class="row">
  <div class="col-sm-12">
    <div class="card">
      <div class="card-header fw-bolder bg-primary text-white">SETTING BERANDA</div>
      <div class="card-body">
        <table class="w-100" id="tables">
          <thead class="text-white border-0" style="background: #5D87FF">
            <tr class="border-0">
              <td class="all text-left px-3 py-3" style="border: 1px solid #5D87FF !important">Tagline</td>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Update Beranda Tagline</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form id="update">
          <div class="mb-3">
            <label for="tagline_home" class="form-label">Tagline</label>
            <textarea name="tagline_home" id="tagline_home" cols="30" rows="6" autocomplete="off" class="form-control"></textarea>
          </div>
          <div>
...
<script>
        ...
        },
        {
          ...
          render: function(data, type, row, meta) {
            return `<div class="d-flex justify-content-center align-items-center gap-1">
              <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#update">Update</button>
            </div>`
          }
        }
      ],
  ...
  $('form#update').submit(function(e) {
    e.preventDefault();
    $.ajax({
      url: '{{ route("setting.home.update", "tagline_home") }}',
      method: 'PUT',
      data: {
        _token: '{{ csrf_token() }}',
        tagline_home: $('#tagline_home').val()
      },
      beforeSend: function() {
